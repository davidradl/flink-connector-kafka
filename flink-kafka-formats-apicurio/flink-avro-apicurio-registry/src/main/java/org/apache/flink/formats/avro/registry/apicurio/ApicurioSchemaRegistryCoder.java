...
public class ApicurioSchemaRegistryCoder implements SchemaCoder {
    ...
    protected long getSchemaId(InputStream in) throws IOException {
        ...
        boolean isKey = dataInputStream.readByte() == ApicurioRecordBasedDeserialization.KEY ? true : false;
        Map<String, byte[]> headersMap = new ApicurioRecordBasedDeserialization().deserializeToHeaders(dataInputStream);
        ...
        if (useHeaders) {
            ...
            String contentIDHeaderName =
                    isKey ? SerdeHeaders.HEADER_KEY_CONTENT_ID : SerdeHeaders.HEADER_VALUE_CONTENT_ID;
            ...
            if (globalIDHeader != null) {
                // if we have a globalId use it, as this is the most accurate way to get a unique schema
    ...
    public void writeSchema(Schema schema, OutputStream out) throws IOException {
        ...
        Object artifactIdObject = configs.get(AvroApicurioFormatOptions.REGISTERED_ARTIFACT_ID.key());
        ...
        if ((boolean) configs.getOrDefault(AvroApicurioFormatOptions.REGISTER_SCHEMA.key(), false)) {

        }
        artifactName = (artifactName == null) ? artifactId : artifactName;

        // config option has a default so no need to check for null
        String artifactDescription = (String) configs.get(
        ...
        if ((boolean) configs.getOrDefault(AvroApicurioFormatOptions.REGISTER_SCHEMA.key(), false)) {
             artifactMetaData = registryClient.createArtifact(
                            groupId,
                            artifactId,
                            artifactVersion,
                            ArtifactType.AVRO,
    ...
    /**
     * Augment the output stream to add schema information.
     * Take the output stream and prepend information:
     * <ul>
     *   <li>the version and format name</li>
     *   <li>the header count, followed by a header id and header value</li>
     *   <li>if useHeaders are false, then add a magic byte and schema id</li>
     * </ul>
     *
     ...
     */
    public static void augmentOutputStream(OutputStream out, boolean useHeaders,
                                           AvroFormatOptions.AvroEncoding avroEncoding,
                                           Long globalId, Long contentId) throws IOException {
