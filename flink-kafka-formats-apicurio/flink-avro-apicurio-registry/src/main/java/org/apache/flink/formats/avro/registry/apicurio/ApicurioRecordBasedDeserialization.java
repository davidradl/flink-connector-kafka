...
public class ApicurioRecordBasedDeserialization implements RecordBasedDeserialization {
    ...
    private byte[] serializeConsumerRecord(
            ConsumerRecord<byte[], byte[]> record, byte keyValueIndicator) {
    ...
    /**
     * Get Kafka header information from the input stream. Expects an input stream of byte : headers
     * count then for each header: int : header name length byte[] : header name int : header value
     * length value[] : header value
     *
    ...
    Map<String, byte[]> deserializeToHeaders(DataInputStream dataInputStream) throws IOException {
        ...
        // throw new RuntimeException("deserializeToHeaders headersCount = " + headersCount +
