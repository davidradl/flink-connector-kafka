...
public class KafkaFactoryUtil {
     ...
     * create an instance of RecordBasedDeserialization. This interface has a canProcess method, which is called
     * in case we have picked up a class that cannot process the Consumer Record.
     *
    ...
    public static <R, F> R loadAndInvokeFactoryWithConsumerRecord(
        ...
        final List<R> instantiatedResources = getInstantiatedResources(factoryInterface, factoryInvoker, errorsDuringInitialization);
        ...
        if (instantiatedResources.size() > 0) {
            ...
            for (int i = 0; i < instantiatedResources.size(); i++) {
                try {
                    ...
                    // called an implementation that threw an Exception while trying to process the consumer record.
                    // TODO log?
                }
            }
        }
    ...
    private static <R, F> List<R> getInstantiatedResources(Class<F> factoryInterface, FactoryInvoker<F, R> factoryInvoker, List<Exception> errorsDuringInitialization) {
