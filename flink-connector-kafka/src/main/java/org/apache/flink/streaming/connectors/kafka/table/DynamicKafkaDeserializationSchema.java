...
/** Dynamic Kafka Deserialization Schema.
...
public class DynamicKafkaDeserializationSchema implements KafkaDeserializationSchema<RowData> {
    ...
    /**
     * Get the Kafka payload for a key. By default this will be the key, but if we can find a RecordBasedDeserialization with identifier RECORD,
     * then that class will specify the payload
     * @param record
    ...
    private byte[] getPayloadForKey(ConsumerRecord<byte[], byte[]> record) throws IOException {
        RecordBasedDeserialization recordBasedDeserialization = getRecordBasedDeserializationFactory(record);
        return recordBasedDeserialization.getSerializedKeyFromConsumerRecord(record);
    }

    /**
     * Get the Kafka payload for a value. By default this will be the value, but if we can find a RecordBasedDeserialization with identifier RECORD,
     * then that class will specify the payload
     * @param record
     * @return the appropriate Kafka payload
     * @throws IOException IO exception occurred
    ...
    private static RecordBasedDeserialization getRecordBasedDeserializationFactory(ConsumerRecord<byte[], byte[]> record) {
        return  KafkaFactoryUtil.loadAndInvokeFactoryWithConsumerRecord(
                RecordBasedDeserializationFactory.class, RecordBasedDeserializationFactory::create,
