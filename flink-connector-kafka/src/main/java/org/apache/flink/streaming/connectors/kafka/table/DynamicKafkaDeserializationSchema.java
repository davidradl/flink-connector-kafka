...
class DynamicKafkaDeserializationSchema implements KafkaDeserializationSchema<RowData> {
    ...
    private void doDeserialize(ConsumerRecord<byte[], byte[]> record, Map<String, Object> additionalParameters, Collector<RowData> collector, boolean isKey) throws IOException {
    ...
    private void performDeserialiserialize(ConsumerRecord<byte[], byte[]> record, Collector<RowData> collector, Map<String, Object> additionalParameters) throws IOException {
        if (deserializeWithAdditionalPropertiesMethod == null) {
            valueDeserialization.deserialize(record.value(), collector);
        } else {
